...
import torch
import fairseq
from fairseq.file_io import save_json, save_npy
...
def eval_lm(
    ...
    for model in models:
        ...
        with torch.profiler.profile(
            ...
            profile_memory=True,
            on_trace_ready=torch.profiler.tensorboard_trace_handler('/home/home1/hh219/moenet/exploration/benchmark/time_profile/torch_trace_6')
        ) as p: # Torch profiler
    ...
    for i, sample in enumerate(batch_iterator):
        ...
        gen_timer.start()
        hypos, expert_assignment_tuple = scorer.generate(models, sample) # Generates the score using the models
        gen_timer.stop(sample["ntokens"])
        expert_assignments_1 += expert_assignment_tuple[0]
        expert_assignments_2 += expert_assignment_tuple[1]
    ...
    # concatenate expert assignments together
    
    expert_assignments_1 = torch.cat(expert_assignments_1, dim=1).cpu().numpy() # Shape is (L)ayer, (S)equence, (E)xperts
    expert_assignments_2 = torch.cat(expert_assignments_2, dim=1).cpu().numpy() # Shape is (L)ayer, (S)equence, (E)xperts
    ...
    }, expert_assignments_1, expert_assignments_2
...
def eval_dataset(cfg: DictConfig, eval_split, task, models, start_time):
    """Evaluate a split of a dataset using all the models"""
    ...
    results, expert_assignments_1, expert_assignments_2 = eval_lm(
        models=models,
    ...
    return results, rr, end_time, expert_assignments_1, expert_assignments_2


def main(cfg: DictConfig, **unused_kwargs):
    print((metrics.get_nvidia_smi_gpu_memory_stats_str())) # record gpu usage
    start_time = time.time()
    ...
    # Load ensemble -- HM: models are initialized here
    ...
    for eval_split in eval_splits:
        results, rr, end_time, expert_assignments_1, expert_assignments_2 = eval_dataset(cfg, eval_split, task, models, start_time)
        start_time = end_time
        all_split_results[eval_split] = rr
        all_expert_assignments.append((expert_assignments_1, expert_assignments_2)) # FIXME: only save one batch

    if isinstance(cfg.eval_lm.stats_path, str):
        save_path = f'{cfg.eval_lm.stats_path}.json' # Results will be saved to this location
        if not torch.distributed.is_initialized() or torch.distributed.get_rank() == 0:
            save_json(all_split_results, save_path)
            logger.info('Evaluation results saved to {}'.format(save_path))
    
    try:
        logger.info(f"Expert assignment dumps to {cfg.eval_lm.experts_path}")
    except omegaconf.errors.ConfigAttributeError:
        cfg.eval_lm.experts_path = "/usr/xtmp/hyhuang/large_models/expert_15b_wiki/expert_assignment"
    if isinstance(cfg.eval_lm.experts_path, str):
        save_path_1 = f"{cfg.eval_lm.experts_path}_1.npy" # Results will be saved to this location
        save_path_2 = f"{cfg.eval_lm.experts_path}_2.npy" # Results will be saved to this location
        if not torch.distributed.is_initialized() or torch.distributed.get_rank() == 0:
            save_npy(expert_assignments_1, save_path_1)
            save_npy(expert_assignments_2, save_path_2)
            logger.info('Expert assignment results saved to {}'.format(cfg.eval_lm.experts_path))
    return results
